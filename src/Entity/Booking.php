...
class Booking
{
     ...
     * @Assert\GreaterThan("today",message="La date d'arrivée doit être ultérieure
     à la date d'aujourd'hui", groups={"front"})
     */
    private $startDate;
     ...
     * @Assert\GreaterThan(propertyPath="startDate", message="La date de départ
    doit être plus éloignée que la date d'arrivée !")
     */
    private $endDate;
    ...
    public function isBookablesDates()
    {
        ...
        $noAvailable = array_map($formatDay, $notAvailablesDays);

        foreach ($days as $day) {
            if (array_search($day, $noAvailable) !== false) return false;
        }
        return true;
    }
